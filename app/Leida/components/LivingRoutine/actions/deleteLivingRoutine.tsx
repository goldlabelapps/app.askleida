...
export const deleteLivingRoutine = (clientId: string): any =>
    async (dispatch: T_UbereduxDispatch, getState: () => T_RootState) => {
        try {
            ...
            if (!patchRes.ok) {
                throw new Error(patchJson?.message || `Could not delete living routine for ${normalizedClientId}`);
            }
            ...
            dispatch(setFeedback({
                title: 'Living Routine Delete Failed',
                description: msg,
                severity: 'error',
            }));
