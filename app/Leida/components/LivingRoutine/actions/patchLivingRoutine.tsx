...
export const patchLivingRoutine = (
    clientId: string,
    routinePatch: Record<string, unknown>,
): any =>
    async (dispatch: T_UbereduxDispatch, getState: () => T_RootState) => {
        try {
            ...
            if (!patchRes.ok) {
                throw new Error(patchJson?.message || `Could not update living routine for ${normalizedClientId}`);
            }
            ...
            dispatch(setFeedback({
                title: 'Living Routine Update Failed',
                description: msg,
                severity: 'error',
            }));
